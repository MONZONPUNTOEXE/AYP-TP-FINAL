...
class CarritoDeCompras:
    ...
    def agregar_producto(self, producto_id, nombre, cantidad, precio, descuento=0):
        # Obtener la fecha de hoy para agregarla
        fecha = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        # Verificar si el producto ya está en el carrito
        if producto_id in self.product_list:
            # Si ya está en el carrito, solo actualizamos la cantidad
            self.product_list[producto_id]["cantidad"] += cantidad
            print(
                f"Se han agregado {cantidad} unidades del producto '{
                    nombre}' al carrito."
            )
        else:
            # Si no está, agregamos el producto con todos sus detalles
    ...
    def mostrar_carrito(self):
        ...
        if self.product_list:
            ...
            for producto_id, detalles in self.product_list.items():
                ...
                total = precio_con_descuento * cantidad  # Total por el producto

                print(
                    f"ID: {producto_id}, Nombre: {nombre}, Cantidad: {
...
# Agregar productos con nombre, cantidad, precio y descuento
